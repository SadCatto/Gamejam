...
func startScenes():
	if(finished==true && scene==1):
		makeVisible($Scene1, $Scene1/Bunny1, $Scene1/Label, ["' Momma always told me that i was a cute little bunny '"])
	...
	if(finished==true && scene==4):
		...
		makeVisible($Scene4, $Scene4/Bunny1, $Scene4/Label, ["' Clearly the pure white race of rabbits is superior, we should rule the world, preach our ways ' "])
	...
	if(finished==true && scene==6):
		...
		makeVisible($Scene6, $Scene6/Bunny1, $Scene6/Label, ["The rage in his eyes could be felt across all 9 realms, 'Release the bunnyKillInator'"])
